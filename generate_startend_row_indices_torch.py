...
def startend_row_indices_to_attn_bias(startend_row_indices, seqlen_q, nheads, dtype, causal=True):
    ...
    m = torch.zeros((bz, num_head, seqlen_q, seqlen_k), dtype=dtype, device=device)
    ...
    m = torch.repeat_interleave(x=m, repeats=nheads // num_head, axis=1)
    return m
...
def generate_sliding_window_mask(batch_size, seqlen_q, seqlen_k, h, window_size=None):
    ...
    startend_row_indices = torch.arange(
        window_size, seqlen_k + window_size, dtype=torch.int32, device=device
...
def generate_causal_document_mask(batch_size, seqlen_q, seqlen_k, h, doc_seqlens=None):
    ...
    # startend_row_indices = torch.as_tensor(startend_row_indices, dtype=torch.int32).reshape((1, 1, seqlen_k, 1)).repeat_interleave(batch_size, 0)
    ...
    startend_row_indices = torch.clip(startend_row_indices, max=seqlen_q)
...
def generate_document_mask(batch_size, seqlen_q, seqlen_k, h, doc_seqlens=None):
    ...
    down_left_row_indices = torch.as_tensor(down_left_row_indices, dtype=torch.int32, device=device).reshape((1, 1, seqlen_k, 1)).repeat_interleave(batch_size, 0)
    up_right_row_indices = torch.as_tensor(up_right_row_indices, dtype=torch.int32, device=device).reshape((1, 1, seqlen_k, 1)).repeat_interleave(batch_size, 0)
    startend_row_indices = torch.concat([down_left_row_indices, up_right_row_indices], axis=-1)
    startend_row_indices = torch.clip(startend_row_indices, max=seqlen_q)
...
def generate_share_question_mask(batch_size, seqlen_q, seqlen_k, h, doc_seqlens=None):
    ...
    startend_row_indices = torch.as_tensor(startend_row_indices, dtype=torch.int32, device=device).reshape((1, 1, seqlen_k, 1)).repeat_interleave(batch_size, 0)
    startend_row_indices = torch.clip(startend_row_indices, max=seqlen_q)
...
def generate_global_sliding_window_mask(batch_size, seqlen_q, seqlen_k, h, global_token=16, window_size=None):
    ...
    down_left_start_row_indices = torch.arange(
        left_window_size + 1, seqlen_k + left_window_size + 1, dtype=torch.int32, device=device
    ).clip(max=seqlen_q)
    ...
    down_left_end_row_indices = torch.full([seqlen_k], seqlen_q, dtype=torch.int32, device=device).reshape((1, 1, seqlen_k, 1)).repeat_interleave(batch_size, 0)

    up_right_start_row_indices = torch.full([seqlen_k], global_token, dtype=torch.int32, device=device)
    ...
    up_right_end_row_indices = torch.arange(
        -right_window_size, seqlen_k - right_window_size, dtype=torch.int32, device=device
    )
    ...
    startend_row_indices = torch.concat([down_left_start_row_indices, down_left_end_row_indices, up_right_start_row_indices, up_right_end_row_indices], axis=-1)
    startend_row_indices = torch.clip(startend_row_indices, max=seqlen_q)
...
def generate_causal_blockwise_mask(batch_size, seqlen_q, seqlen_k, h, doc_seqlens=None):
    ...
    start_row_indices = torch.as_tensor(start_row_indices, dtype=torch.int32, device=device).reshape((1, 1, seqlen_k, 1)).repeat_interleave(batch_size, 0)

    seq_cusums = np.cumsum(doc_seqlens)
    end_row_indices = [seq_cusums[-2]] * seq_cusums[-2] + [seq_cusums[-1]] * doc_seqlens[-1] + [seqlen_k] * padding
    end_row_indices = torch.as_tensor(end_row_indices, dtype=torch.int32, device=device).reshape((1, 1, seqlen_k, 1)).repeat_interleave(batch_size, 0)

    startend_row_indices = torch.concat([start_row_indices, end_row_indices], axis=-1)
    startend_row_indices = torch.clip(startend_row_indices, max=seqlen_q)
...
def generate_prefix_lm_causal_mask(batch_size, seqlen_q, seqlen_k, h, prefix_length=None):
    ...
    down_left_row_indices = torch.full([seqlen_k], seqlen_k, dtype=torch.int32, device=device).reshape((1, 1, seqlen_k, 1)).repeat_interleave(batch_size, 0)
    up_right_row_indices = torch.as_tensor([0] * prefix_length + list(range(prefix_length, seqlen_k)), dtype=torch.int32, device=device).reshape((1, 1, seqlen_k, 1)).repeat_interleave(batch_size, 0)
    startend_row_indices = torch.concat([down_left_row_indices, up_right_row_indices], axis=-1)
    startend_row_indices = torch.clip(startend_row_indices, max=seqlen_q)
...
def generate_random_eviction_mask(batch_size, seqlen_q, seqlen_k, h, start_row=None):
    ...
    startend_row_indices = torch.as_tensor(start_rows_list, dtype=torch.int32, device=device).reshape((batch_size, h, seqlen_k, 1))
    startend_row_indices = torch.clip(startend_row_indices, max=seqlen_q)
    causal = True
    return startend_row_indices, causal
