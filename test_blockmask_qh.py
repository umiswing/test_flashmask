...
def generate_shapes():
    for batch_size, seqlen_q, seqlen_k, nheads, nheads_kv in shape_cases:
        # flashmask historically supported `num_head` = 1 or `nheads_kv`.
        # We also want to test `num_head` = `nheads` (i.e. `nheads_q`).
        nheads_startend_row_indices_values = list(dict.fromkeys([1, nheads_kv, nheads]))
...
def _expand_startend_row_indices_heads(startend_row_indices, target_num_heads: int):
    if startend_row_indices is None:
        return None
    if target_num_heads <= 0:
        raise ValueError(f"Invalid target_num_heads={target_num_heads}")
    _, num_head, _, _ = startend_row_indices.shape
    if num_head == target_num_heads:
        return startend_row_indices
    if target_num_heads % num_head != 0:
        raise ValueError(
            f"Cannot expand startend_row_indices head dim from {num_head} to {target_num_heads}"
        )
    repeat_factor = target_num_heads // num_head
    return startend_row_indices.repeat_interleave(repeat_factor, axis=1)
...
def test_flashmask(
    ...
    startend_row_indices = _expand_startend_row_indices_heads(
        startend_row_indices, nheads_startend_row_indices
    )
    assert startend_row_indices is None or startend_row_indices.shape[1] == nheads_startend_row_indices
    ...
    assert blockmask.shape[1] == nheads_startend_row_indices
    # print(q_ref)
    ...
    attn_bias = paddle.where(
        mask_inf,
        paddle.zeros_like(mask_inf, dtype=paddle.float32),
        paddle.full_like(mask_inf, float("-inf"), dtype=paddle.float32),
    )
