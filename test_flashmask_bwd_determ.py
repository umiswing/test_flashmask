...
def test_flashmask(
    ...
    # --- 3. 反向传播确定性验证---
    # if is_deterministic:
    print("\n--- Verifying Backward Pass Determinism ---")

    # 保存第一次运行的梯度作为基准
    dq_first = q.grad.detach().clone()
    ...
    # 打印与第一次运行结果的差异
    print(f'dQ max diff with first run: {(dq2 - dq_first).abs().max().item()}')
    print(f'dK max diff with first run: {(dk2 - dk_first).abs().max().item()}')
    print(f'dV max diff with first run: {(dv2 - dv_first).abs().max().item()}')

    # 断言：本次梯度必须与基准梯度逐位完全相等
    assert dq2._md5sum() == dq_first._md5sum(), f"dQ not deterministic"
    assert dk2._md5sum() == dk_first._md5sum(), f"dK not deterministic"
    assert dv2._md5sum() == dv_first._md5sum(), f"dV not deterministic"
