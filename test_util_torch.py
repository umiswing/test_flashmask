...
def attention_ref(
    ...
    Arguments:
        ...
        upcast: whether to to all inputs to fp32, do all computation in fp32, then to
    ...
    if q_descale is not None:
        ...
        qv = (qv.to(torch.float32) * q_descale).to(qv.dtype) if qv is not None else None

    if k_descale is not None:
        assert False
        k = (k.to(torch.float32) * rearrange(k_descale, "b h -> b 1 h 1")).to(k.dtype)

    if v_descale is not None:
        assert False
        v = (v.to(torch.float32) * rearrange(v_descale, "b h -> b 1 h 1")).to(v.dtype)
    ...
    attention = torch.softmax(scores, axis=-1).to(v.dtype)
    ...
    if local_mask is not None:
        attention = attention.masked_fill(torch.all(local_mask, axis=-1, keepdim=True), 0.0)
    dropout_scaling = 1.0 / (1 - dropout_p)
    # attention_drop = attention.masked_fill(~dropout_mask, 0.0) * dropout_scaling
    # output = torch.matmul(attention_drop, v, transpose_y=True)
...
def random_blockmask(shape, dtype='int32',is_causal=False, ref_q = None):
    # 随机生成 0/1 mask
    mask = torch.randint(0, 2, shape, dtype=torch.int32)
    B, S, Q, K = shape
    return mask
...
def flashmask_to_densemask(startend_row_indices, seqlen_q, nheads, causal=True):
    ...
    m = torch.ones((bz, num_head, seqlen_q, seqlen_k), dtype=torch.int32)
...
def blockmask_to_densemask(blockmask, q_len, k_len, dtype, causal=True):
    ...
    return densemask.astype(torch.bool)
