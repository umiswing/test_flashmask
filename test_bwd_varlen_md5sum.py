...
fa_versions = [4]
# head_dim must be a multiple of 32 for varlen backward (see assert in
# _flash_attn_bwd). Keep in sync with test_flashmask_use_varlen.py.
d_dv_combinations = [
    (64, 64),
    (128, 128), 
    (192, 128),
...
def run_flashmask_varlen_backward(
    ...
    # Use out.shape to accommodate dv != d.
...
def record_gt(output_file="flashmask_varlen_bwd_gt.json"):
    gt_records = {}
    ...
    for i, params in enumerate(param_combinations):
        try:
            dq_md5, dk_md5, dv_md5 = run_flashmask_varlen_backward(**params)
            ...
            continue
    gt_records["gt_commit_id"] = input("Please input the commit ID of varlen bwd GT md5sum: ")
    gt_records["gt_commit_msg"] = input("Please input the commit msg of varlen bwd GT md5sum: ")
...
gt_records = {}
try:
    with open("flashmask_varlen_bwd_gt.json", 'r') as f:
...
def test_flashmask_varlen_bwd_md5(
    batch_size, seqlen_q, seqlen_k, nheads, nheads_kv, d, dv,
    nheads_startend_row_indices, fa_version, dtype, mask_type,
    gen_startend_row_indices, softmax_scale, softcap=0.0,
):
...
if __name__ == "__main__":
    if not os.path.exists("flashmask_varlen_bwd_gt.json"):
