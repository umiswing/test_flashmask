"""
Performance benchmark for flash_attn_varlen_func (FA4 varlen).

Causal Document Mask (FA4 mask_mod) and flash_attn_varlen are equivalent:
...
This script benchmarks flash_attn_varlen_func across a range of configs
that mirror the shape / doc-distribution used in benchmark_fa4_mask_mod.py,
so numbers can be compared directly.

Usage:
    python tests/cute/benchmark_varlen.py
    python tests/cute/benchmark_varlen.py --dtype fp16
    python tests/cute/benchmark_varlen.py --seqlens 2048 4096 8192 --D 128
    python tests/cute/benchmark_varlen.py --from_file kernel_test_seq_info.txt
"""
...
# ---------------------------------------------------------------------------
# Benchmarking helpers  (mirror benchmark_fa4_mask_mod.py)
# ---------------------------------------------------------------------------
...
def do_bench(fn, warmup=10, rep=50, grad_to_none=None,
    ...
    cache = torch.empty(int(256 * 1024 * 1024 // 4), dtype=torch.int32, device="cuda")
...
def build_varlen_tensors(
    ...
    q = torch.randn(total, H,   D,  device=device, dtype=dtype, requires_grad=True)
    k = torch.randn(total, HKV, D,  device=device, dtype=dtype, requires_grad=True)
    v = torch.randn(total, HKV, DV, device=device, dtype=dtype, requires_grad=True)

    lens_t  = torch.tensor(doc_seq_lens, dtype=torch.int32, device=device)
    cu_seqlens = torch.cat(
        [torch.zeros(1, dtype=torch.int32, device=device),
         lens_t.cumsum(0).to(torch.int32)]
    )
...
def benchmark_varlen(
    ...
    Equivalent to the Causal Document Mask benchmark in benchmark_fa4_mask_mod.py.
    """
    seq_lens, max_seqlen = causal_doc_mask_to_varlen(doc_seq_lens, S)

    q, k, v, cu_seqlens, max_seqlen = build_varlen_tensors(
        seq_lens, H, HKV, D, DV, dtype, device
    )
    ...
    # no autograd state, no graph overhead.
    out, lse = fwd_fn()
    ...
    # matches benchmark_fa4_mask_mod.py cal_flops convention
...
# ---------------------------------------------------------------------------
# Doc-len generators  (mirrors benchmark_fa4_mask_mod.py helpers)
# ---------------------------------------------------------------------------
...
def main(
    ...
    if from_file is not None:
        # Same format as benchmark_fa4_mask_mod.py reads kernel_test_seq_info.txt
        total_length = 0
    ...
    for D in [128, 192]:
        ...
        for idx, (S, doc_seq_lens) in enumerate(configs):
            B = 128 * 1024 // S  # match benchmark_fa4_mask_mod convention
            ...
            # matching benchmark_fa4_mask_mod which processes B padded samples.
            ...
            os.makedirs(dtype, exist_ok=True)
            text_file = open(f"{dtype}/fa4_varlen_torch_{current_time}_{B}_{S}_{H}_{HKV}_{D}_{DV}_{idx}.csv", "w")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Benchmark flash_attn_varlen_func (FA4 varlen).\n"
    ...
    parser.add_argument(
        ...
        help="Read doc-seq-len configs from a kernel_test_seq_info.txt file "
             "(same format as benchmark_fa4_mask_mod.py).",
    )
    parser.add_argument(
